...
public class PersianDate {
     ...
     */
    public PersianDate() {
        this.timeInMilliSecond = removeMilliSecond(System.currentTimeMillis());
        this.changeTime();
    }
    ...
    public PersianDate(Long timeInMilliSecond) {
        this.timeInMilliSecond = removeMilliSecond(timeInMilliSecond);
        this.changeTime();
    }
    ...
    public PersianDate(Date date) {
        this.timeInMilliSecond = removeMilliSecond(date.getTime());
        this.changeTime();
    }
     ...
     * @return
     */
    public PersianDate initDateByGrg(int grgYear, int grgMonth, int grgDay) {
        return this.initDateByGrg(grgYear, grgMonth, grgDay, 0, 0, 0);
    }
     ...
     * @return
     */
    public PersianDate initDateByJalali(int shYear, int shMonth, int shDay) {
        return this.initDateByJalali(shYear, shMonth, shDay, 0, 0, 0);
    }
     ...
     * @return
     */
    public PersianDate initDate(long timeInMilliSecond) {
    ...
    private void prepareDate() {
        int[] toGrg = this.toGregorian(this.getYear(), this.getMonth(), this.getDay());
        String dtStart = "" + this.textNumberFilter("" + toGrg[0]) + "-" + this.textNumberFilter("" + toGrg[1]) + "-" + this.textNumberFilter("" + toGrg[2])
        ...
        try {
            this.timeInMilliSecond = removeMilliSecond(format.parse(dtStart).getTime());
...
//		return ((Year % 4) == 0);
    ...
    public PersianDate addDate(long year, long month, long day, long hour, long minute, long second) {
        ...
        for (long i = (year - 1); i >= 0; i--) {
            if (this.isLeap(this.getYear() + (int) i)) {
                day += 366;
            } else {
                day += 365;
            }
        ...
        for (long i = (month - 1); i >= 0; i--) {
            int monthTmp = this.getMonth() + (int) i;
            int yearTmp = this.getYear();
            if (monthTmp > 12) {
                monthTmp -= 12;
                yearTmp++;
            }
    ...
    public static PersianDate today() {
        ...
        return pdate.initDateByJalali(pdate.getYear(), pdate.getMonth(), pdate.getDay(), 0, 0, 0);
    }
    ...
    public static PersianDate tomorrow() {
        ...
        return persianDate.initDateByJalali(persianDate.getYear(), persianDate.getMonth(), persianDate.getDay() + 1, 0, 0, 0);
    }
    ...
    public PersianDate startOfDay(PersianDate persianDate) {
        return persianDate.initDateByJalali(persianDate.getYear(), persianDate.getMonth(), persianDate.getDay(), 0, 0, 0);
    }
    ...
    public PersianDate endOfDay(PersianDate persianDate) {
        return persianDate.initDateByJalali(persianDate.getYear(), persianDate.getMonth(), persianDate.getDay(), 23, 59, 59);
    }
    ...
    public Integer getMonthLength(PersianDate persianDate) {
        return this.getMonthLength(persianDate.getYear(), persianDate.getMonth());
    }
